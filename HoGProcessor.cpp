...
CvMat* HoGProcessor::svm_predict(CvSVM *svm, CvMat *input){
	//printf("Predicting using SVM\n");	
	CvMat* output = cvCreateMat(input->height, 1, CV_8U);
	CvMat* sample = cvCreateMat(1, 2, CV_32F);
	for(int row = 0; row < input->height; row++)
	{
		sample = cvGetRow(input, sample,row);
		double predicted_px = svm->predict(sample);
		cvSet2D(output, row,0, cvScalar(predicted_px));
	}
	return output;
}

void HoGProcessor::detectObject(CvSVM *svmModel,IplImage** integrals , IplImage *input, IplImage *result, CvRect window,int normalization){
	...
	int i,j;
	for(i = -nWindow; i < nWindow; i++)
		for(j = -nWindow; j < nWindow; j++)
		{			
			rect.x = window.x + j;
			rect.y = window.y + i;
			if(rect.x < 0 || rect.y <0)
				continue;
			if(rect.x + window.width > result->width ||
				rect.y + window.height > result->height )
				continue;
			
			CvMat* img_feature_vector ;

			img_feature_vector = calculateHOG_window(integrals, rect, normalization);					
			
			double predict_rs =svmModel->predict(img_feature_vector);
				
			if(predict_rs != 2)
			{
				cvRectangle(result, cvPoint(rect.x, rect.y), cvPoint(rect.x + rect.width,rect.y + rect.height), CV_RGB(255,0,0), 2);				
...
CvRect HoGProcessor::detectObject(CvSVM *svmModel, IplImage *input, CvPoint blob_center,int normalization)
{		
	int StepWidth = 5;
	int StepHeight = 5;
	...
	for(int i = -nWindow; i <= nWindow; i++)
	{
		...
		{
			CvRect rect;
			rect.x = blob_center.x - (windowWidth + StepWidth*i)/2;
			rect.y = blob_center.y - (windowHeight + StepHeight*j)/5;
			rect.width = windowWidth + StepWidth*i;
			rect.height = windowHeight + StepHeight*j;

			if(rect.x < 0 || rect.y <0)
				continue;
			if(rect.x + windowWidth + StepWidth*i > input->width ||
				rect.y + windowHeight + StepHeight*j > input->height )
				continue;
	...
	{		
		return MergeRect(lstRect);
				//cout<<"list rect size: " <<lstRect.size()<<" size of rect : (" <<resultRect.x<<" , "<<resultRect.y<<" , "<<resultRect.width<<" , "<<resultRect.height<<" )"<<endl;		
	}
...
void HoGProcessor::detectObject(CvSVM *svmModel, IplImage *input, IplImage *result,CvSize cell, CvSize window,int normalization){
	//Tinh integrals image	
	IplImage** integrals = calculateIntegralHOG(input);
		
	CvMat* img_feature_vector ;
	
	for (int row = 0; row < input->height - window.height; row+= 20)
	{
		for (int col = 0; col < input->width - window.width; col+= 30)
		{
			img_feature_vector = calculateHOG_window(
			integrals, cvRect(col,row, window.width, window.height), normalization);					

			double predict_rs =svmModel->predict(img_feature_vector);
		
			if(predict_rs != 2)
			{
				cvRectangle(result,cvPoint(col,row),cvPoint(col+window.width,row + window.height),cvScalar(255,0,0));				
			}		

			cvReleaseMat(&img_feature_vector);	
		}
	}
	cvReleaseImage(integrals);	
}
...
/*
IplImage* ComputeGradient(IplImage *grayI)
{
	CvSize size = cvGetSize(grayI);
	int depth = grayI->depth;

	IplImage *dilate = cvCreateImage(size, depth, 1);
	IplImage *erode = cvCreateImage(size, depth, 1);
	IplImage *mask = cvLoadImage("E://Data//mask.png",CV_LOAD_IMAGE_GRAYSCALE);
	IplImage *result = cvCreateImage(size, depth, 1);
	
	//gradient = diale - erosion	
	cvDilate(grayI, dilate);
	cvErode(grayI, erode);

	cvSub(dilate, erode, result);
	...
	for(i=0;i<row;i++)
	{
		for(j=0;j<col;j++)
		{
			uchar* pixel = &CV_IMAGE_ELEM(mask,uchar,i,j);
			if(pixel[0] == 0)
			{
				uchar* pixelR = &CV_IMAGE_ELEM(result,uchar,i,j);
				pixelR[0] = 0;
			}			
		}
	}
