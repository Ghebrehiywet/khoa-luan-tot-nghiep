#include "cv.h"
#include "highgui.h"
#include "stdio.h"
#include "time.h"
#include "HoGProcessor.h"
#include "GaussFilterColor.h"
#include "vector_space.h"
#include "snake.h"
#include "snakewindow.h"


#define _VIDEO_ "data\\video\\video.avi"
#define _MODEL_PATH "data\\model.xml"
#define _COLOR_MODEL "data\\out.txt"
#define TRAIN_DATA 0
#define TRAIN_HAIR 0


#define distanceMin 25


vector<CvRect> ConnectOverlapRects(vector<CvRect> inVector){
	vector<CvRect> result;
	vector<CvRect>::iterator iter1, iter2;

	iter1 = inVector.begin();
	while(iter1 != inVector.end() - 1){
		if(iter1->width >= 0){
			iter2 = iter1 + 1;
			while(iter2 != inVector.end()){
				CvRect tmp = cvRectIntersection(*iter1, *iter2);				
				if(tmp.width > 0 && tmp.height>0){
					//intersection happen
					int areaOverlap = tmp.width * tmp.height;
					int area1 = iter1->width * iter1->height;
					int area2 = iter2->width * iter2->height;
					if(areaOverlap*1.0/area1 >= 0.7 || areaOverlap*1.0/area2 >= 0.7){
						//merge rect
						iter1->x = (iter1->x < iter2->x)?iter1->x:iter2->x;
						iter1->y = (iter1->y < iter2->y)?iter1->y:iter2->y;
						iter1->width = iter1->width + iter2->width - tmp.width;
						iter1->height = iter2->height + iter1->height - tmp.height;

						//remove iter2
						iter2->width = -1;
					}
				}
				iter2++;
			}
			result.push_back(*iter1);		
		}

		iter1++;
	}

	if(iter1->width > 0){
		result.push_back(*iter1);
	}

	return result;
}

void OutputResult(IplImage* img, vector<CvRect> vRect, CvScalar color){
	
	vector<CvRect>::iterator iter;
	for(iter = vRect.begin(); iter != vRect.end(); iter++){
		cvRectangle(img, cvPoint(iter->x, iter->y), cvPoint(iter->x + iter->width,iter->y + iter->height), color, 2);
	}
}

int main2() {
	if(TRAIN_HAIR){
		GaussFilterColor gauss;
		gauss.TrainData("hair",".png", 15, 1, 15, _COLOR_MODEL);
	}

	if(TRAIN_DATA)
	{		
		char* prefixPos = "E:\\Data\\Head\\pos\\pic";
		char* posfixPos = ".png";
		
		int pos_startindex = 1;
		int pos_endindex   = 1;//352;
		int pos_numberofsample = pos_endindex - pos_startindex+1;

		char* prefixNeg = "E:\\Data\\Head\\neg\\pic";
		char* posfixNeg = ".png";

		int neg_startindex = 1;
		int neg_endindex = 1;//425;
		int neg_numberofsample = neg_endindex - neg_startindex + 1;
							 
		HoGProcessor hog;
		
		CvMat* pos_mat = hog.train_64x128(prefixPos, posfixPos, cvSize(48,48), pos_numberofsample, pos_startindex, pos_endindex);
		
		CvMat* neg_mat = hog.train_large(prefixNeg, posfixNeg, cvSize(48,48), neg_numberofsample, 3, 3, neg_startindex, neg_endindex);
		hog.trainSVM(pos_mat, neg_mat, _MODEL_PATH);		

		printf("\n\n");

		printf("Syntax :12 parameter :\n");
		printf("HoG.exe train_model <positive_folder_prefix> <positive_subfix> <positive_start_index> <positive_end_index> <negative_folder_prefix>  <negative_sufix> <negative_startindex> <negative_endindex> <outputfile> <isTestPositive>\n" );
		printf("\n\n");
	}
	else
	{
		// global variables
		IplImage *hair_grayscale, *hair_canny;
		int frame_count = 0;


		FILE *big_data, *medium_data, *small_data;
		big_data = fopen("big.txt", "r");
		medium_data = fopen("medium.txt", "r");
		small_data = fopen("small.txt", "r");

		SnakeWindow *bigw, *mediumw, *smallw;
		bigw = new SnakeWindow(big_data);
		mediumw = new SnakeWindow(medium_data);
		smallw = new SnakeWindow(small_data);


		// init font
		CvFont font;
		cvInitFont(&font, CV_FONT_HERSHEY_SIMPLEX, 0.4, 0.4);



		CvCapture *capture = cvCaptureFromFile(_VIDEO_);
		if (capture == NULL) {
			printf("Cannot open video.\n");
			return EXIT_FAILURE;
		}

		int count = 0;
		float speed;
		time_t start = time(NULL);

		IplImage *frame = cvQueryFrame(capture);
		IplImage *subtract = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);
		IplImage *subtract2 = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);


		hair_grayscale = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);
		hair_canny = cvCreateImage(cvGetSize(frame), IPL_DEPTH_8U, 1);


		int fps = (int)cvGetCaptureProperty(capture, CV_CAP_PROP_FPS);

		cvNamedWindow("Video");
		cvNamedWindow("Canny");

		CvMemStorage *storage = cvCreateMemStorage();
		CvMemStorage *points = cvCreateMemStorage();
		CvSeq *contours = 0;

		CvFont myFont;
		cvInitFont(&myFont,CV_FONT_HERSHEY_COMPLEX_SMALL ,1.5f,1.5f,0,1,8);
		
		HoGProcessor hog;
		CvSVM svm;

		IplImage *result;
		
		CvRect window = cvRect(0,0,48,48);

		result = cvCloneImage(frame);

		svm.load(_MODEL_PATH);
		
		GaussFilterColor gauss;
		gauss.LoadData(_COLOR_MODEL);

		vector<CvRect> vectorRect;

		while (1) {
			frame = cvQueryFrame(capture);
			if (frame == NULL) {
				printf("End video.\n");
				break;
			}	

			cvCvtColor(frame, hair_grayscale, CV_RGB2GRAY);
			cvCanny(hair_grayscale, hair_canny, 10, 200);

			
			subtract = gauss.Classify(frame);
			cvCopy(subtract, subtract2);


			cvSmooth(subtract, hair_canny, CV_MEDIAN);
			cvCanny(hair_canny, hair_canny, 10, 100);

			
			cvFindContours(subtract, storage, &contours, sizeof(CvContour), CV_RETR_EXTERNAL);

			cvCopyImage(frame, result);



			if (contours) {				
				vectorRect.clear();				
				while (contours != NULL) {
					CvRect rectHead = cvBoundingRect(contours);					
					if((rectHead.width / rectHead.height >= 3) 
						|| (rectHead.height / rectHead.width >= 3) 
						|| ((rectHead.width * rectHead.height) < 100)
						|| (rectHead.width * rectHead.height > 10000)){
						contours = contours->h_next;
						continue;
					}

					cvRectangle(subtract, cvPoint(rectHead.x, rectHead.y), cvPoint(rectHead.x + rectHead.width, rectHead.y + rectHead.height), CV_RGB(255,255,255));
					
					CvRect detectedRect = hog.detectObject(&svm, frame, result, rectHead);
					if(detectedRect.width > 0){
						vectorRect.push_back(detectedRect);
					}
										
					contours = contours->h_next;
				}
			}

			vectorRect = ConnectOverlapRects(vectorRect);
			for (unsigned int i = 0; i < vectorRect.size(); i++) {
				CvRect rect = vectorRect.at(i);
				CvPoint location = cvPoint(rect.x+rect.width*1.0f/3, rect.y+rect.height*1.0f/3);
				//Snake *fit_snake = snake_window->GetSnake(result, hair_canny, location);


				Snake *fit_snake;
				SnakeWindow *win;
				int current_y = rect.y+rect.height*1.0f/3;
				int frame_height_step = frame->height*1.0/3;
				if (current_y >= 0 && current_y < frame_height_step) {
					win = smallw;
				}
				else if (current_y >= frame_height_step && current_y < frame_height_step*2) {
					win = mediumw;
				}
				else if (current_y >= frame_height_step*2 && current_y <frame->height) {
					win = bigw;
				}
				fit_snake = win->GetSnake(result, hair_canny, location, rect);

				if (fit_snake != NULL) {
					fit_snake->DrawCurve(result, location);
				}
			}

			OutputResult(result, vectorRect, CV_RGB(255,0,0));


			char number[20];
			itoa(svm.get_support_vector_count(),number,10);

			//Put text on image
			cvPutText( result, number, cvPoint(20,25), &myFont , CV_RGB(0,255,255));
					
			count++;
			cvShowImage("Subtraction", subtract2);
			cvShowImage("Video", result);
			cvShowImage("Canny", hair_canny);
	
			cout<<"end frame "<<count<<endl;
						
			if(cvWaitKey(fps) == 27)
				break;
		}

		time_t end = time(NULL);
		speed = count*1.0f/(end-start);
		printf("fps: %2.3f.\n", speed);

		delete bigw;
		delete mediumw;
		delete smallw;

		fclose(big_data);
		fclose(medium_data);
		fclose(small_data);

		cvDestroyAllWindows();

		cvReleaseCapture(&capture);
		cvReleaseImage(&result);		
		cvReleaseImage(&subtract);		
		cvReleaseMemStorage(&storage);
		cvReleaseMemStorage(&points);
	}
	return EXIT_SUCCESS;
}