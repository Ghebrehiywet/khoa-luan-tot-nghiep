...
bool Utils::CheckRectHead(CvRect rectHead, int iMaxHeight, Head_Params headParams){
	...
	if((rectHead.width / rectHead.height >= headParams.m_iRelative_Width_Height) 
		...
		|| ((rectHead.width * rectHead.height) < headParams.m_iMinHeadArea)
		|| (rectHead.width * rectHead.height > headParams.m_iMaxHeadArea)){		
		return false;
	}

	if(rectHead.y < iMaxHeight/3)
	{		
		if(rectHead.width * rectHead.height >= headParams.m_iMaxHeadAreaTop){
			return false;
		}
	}
	else
	{	
		...
		{			
			if(rectHead.width * rectHead.height <= headParams.m_iMinHeadAreaBottom){
				return false;
			}													
		}	
	}
