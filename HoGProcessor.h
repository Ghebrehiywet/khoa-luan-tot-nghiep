...
public:	
	...
	CvMat* svm_predict(CvSVM* svm, CvMat* input);

	void calculateHOG_block(CvRect block, CvMat* hog_block, 
	IplImage** integrals,CvSize cell, int normalization);
	void detectObject(CvSVM *svmModel, IplImage *input, IplImage *result,CvSize cell, CvSize window,int normalization = 4);
	void detectObject(CvSVM *svmModel,IplImage** intergrals, IplImage *input, IplImage *result, CvRect window,int normalization = 4);
	CvRect detectObject(CvSVM *svmModel, IplImage *input, CvPoint blob_center,int normalization = 4);
